...
export interface DriverCardProps {
  id: string; // ej: "max-verstappen", "michael-schumacher"
  fullName?: string;
  name?: string;
  surname?: string;
  imageUrl?: string; // 👈 Prop para la URL proveniente de Cloudinary / MongoDB
...
export function DriverCard({
  ...
  const driverCountryCode =
    alpha2Code || countryId || fallbackDriverCountryCodes[extractedLastName] || 'GB';
  const countryFlag = getCountryFlag(driverCountryCode);
  const countryFlagUrl =
    countryFlag && countryFlag.startsWith('http') ? countryFlag : undefined;

  // 3. Diccionario de imágenes oficiales de la web de F1 (Parrilla 2026)
  ...
  // 4. Lógica jerárquica de resolución de la URL de la imagen
  const officialF1Image = officialF1DriversImgMap[extractedLastName];

const driverImageUrl = imgError
  ? '/drivers/placeholder.png'
  : officialF1Image || imageUrl || '/drivers/placeholder.svg';

  // 5. Colores e identidades visuales
  ...
  const gradient =
    teamColors[teamName?.toUpperCase() || ''] || 'from-gray-700 to-gray-900';
